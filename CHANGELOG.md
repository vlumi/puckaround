...
- **Change the setup without leaving the table.** Tap the center ring and there's a new "Change setup" button — players, first-to, match length, puck and walls, the same choices as the front page. It's clear what happens next: change something and the button reads "Start new game" (with a note that it will), leave it untouched and it reads "Resume", and Cancel always backs out with the current game untouched. No more quitting to the title just to switch the puck.
