...
- **Practice against the machine.** The title's third mode: you take the bottom end while a mallet on a fixed metronome sweep defends the top goal — a drill target, not an AI. It never aims, but its deflections count, so careless shots come back at you. The Practice button opens the same setup modal as a match (minus Players — it's always you against the machine), so first to 7 against the wiper or three triangles on a wrap table is your pick on the way in, and mid-game the pause menu's "New practice…" re-opens it.
