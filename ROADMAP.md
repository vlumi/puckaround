...
- [ ] **Pace levels.** A setup pick for how fast the table plays (the `Rink.pace` seam makes it nearly free) — parked with doubt: a mallet hit already spans a huge speed range, so a cap preset may barely register. Framed as pace, never difficulty; both sides share the table.
